...
## Descrição
Criar classe de configuração CustomerConfiguration que mapeia a entidade CustomerEntity (persistência) para a tabela Customers no banco de dados. CustomerEntity já possui propriedades como strings, então não precisa conversão de Value Objects (isso será feito no repositório ao mapear entre Domain e Infra).
